...
class UserModel {
    ...
    public function getUserById($userId) {
        try {
            $query = "SELECT u.*, b.bed_label, r.room_code, r.status as room_status, r.max_capacity,
    ...
    /**
     * Lấy tin nhắn cuối cùng giữa 2 user (dùng cho chat sidebar)
     */
    public function getLastMessage(int $userId1, int $userId2): ?array {
        $sql = "SELECT sender_id, recipient_id, content, created_at
                FROM messages
    ...
    public function getUsers($searchParams = []) {
        try {
            $query = "SELECT u.user_id, u.student_id, u.username, u.profile_picture, u.fullname, u.email, u.status, u.role, u.hometown,
                             r.room_code, b.bed_number, f.floor_number
                      FROM users u
                      LEFT JOIN beds b ON u.user_id = b.student_id
                      LEFT JOIN rooms r ON b.room_id = r.id
                      LEFT JOIN floors f ON r.floor_id = f.id
            ...
            if (!empty($searchParams['room_code'])) {
                $query .= " AND r.room_code = ?";
                $params[] = $searchParams['room_code'];
                $types .= 's';
            }
            if (!empty($searchParams['floor_id'])) {
                $query .= " AND r.floor_id = ?";
                $params[] = $searchParams['floor_id'];
                $types .= 'i';
            }
