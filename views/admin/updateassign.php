<?php
// path: views/admin/updateAssignment.php

require_once __DIR__ . '/../../includes/db.php';
require_once __DIR__ . '/../../controllers/DeviceController.php';

// Kiểm tra đăng nhập và quyền admin
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'admin') {
    $_SESSION['error_message'] = 'Bạn cần đăng nhập với quyền admin để thực hiện thao tác này.';
    header("Location: /network-management/auth/login.php");
    exit();
}

// Khởi tạo DeviceController
$deviceController = new DeviceController($_SESSION['user_id'], $_SESSION['role']);

// Lấy và validate assignment_id từ URL
$assignment_id = filter_input(INPUT_GET, 'assignment_id', FILTER_VALIDATE_INT);
if (!$assignment_id) {
    $_SESSION['error_message'] = 'Thiếu hoặc không hợp lệ assignment_id.';
    header("Location: /network-management/views/admin/assignmentlist.php");
    exit();
}

// Xử lý yêu cầu cập nhật phân quyền qua POST
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    header('Content-Type: application/json');

    try {
        // Lấy và lọc dữ liệu từ POST
        $expected_return_date = filter_input(INPUT_POST, 'expected_return_date', FILTER_SANITIZE_STRING);
        $actual_return_date = filter_input(INPUT_POST, 'actual_return_date', FILTER_SANITIZE_STRING);
        $status = filter_input(INPUT_POST, 'status', FILTER_SANITIZE_STRING);
        $notes = filter_input(INPUT_POST, 'notes', FILTER_SANITIZE_STRING);

        // Kiểm tra định dạng ngày
        if ($expected_return_date && !strtotime($expected_return_date)) {
            throw new Exception("Ngày trả dự kiến không hợp lệ (ví dụ: YYYY-MM-DD).");
        }
        if ($actual_return_date && !strtotime($actual_return_date)) {
            throw new Exception("Ngày trả thực tế không hợp lệ (ví dụ: YYYY-MM-DD).");
        }

        // Kiểm tra trạng thái hợp lệ
        $validStatuses = ['active', 'returned', 'update'];
        if ($status && !in_array($status, $validStatuses)) {
            throw new Exception("Trạng thái không hợp lệ. Giá trị cho phép: " . implode(", ", $validStatuses));
        }

        // Tạo mảng dữ liệu, chỉ bao gồm các trường có giá trị
        $data = array_filter([
            'expected_return_date' => $expected_return_date ?: null,
            'actual_return_date' => $actual_return_date ?: null,
            'status' => $status ?: null,
            'notes' => $notes ?: null
        ], fn($value) => $value !== null && $value !== '');

        if (empty($data)) {
            throw new Exception("Không có dữ liệu hợp lệ để cập nhật.");
        }

        // Chuẩn bị tham số cho handleRequest
        $params = [
            'assignment_id' => $assignment_id,
            'data' => $data
        ];

        // Gọi handleRequest
        $result = $deviceController->handleRequest('update_assignment', $params);

        // Phản hồi JSON
        echo json_encode([
            'status' => $result['success'] ? 'success' : 'error',
            'message' => $result['message'],
            'data' => $result['data'] ?? null
        ]);
    } catch (Exception $e) {
        error_log("Lỗi trong updateAssignment.php: " . $e->getMessage());
        echo json_encode([
            'status' => 'error',
            'message' => 'Lỗi: ' . $e->getMessage(),
            'data' => null
        ]);
    }
    exit();
}

// Xử lý thông báo toast
$show_success_toast = isset($_SESSION['success_message']);
$show_error_toast = isset($_SESSION['error_message']);
$success_message = $show_success_toast ? htmlspecialchars($_SESSION['success_message']) : '';
$error_message = $show_error_toast ? htmlspecialchars($_SESSION['error_message']) : '';

if ($show_success_toast) unset($_SESSION['success_message']);
if ($show_error_toast) unset($_SESSION['error_message']);
?>

<!DOCTYPE html>
<html lang="vi">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cập nhật thông tin phân quyền</title>
    <link rel="shortcut icon" type="image/x-icon" href="../../assets/img/favicon.svg">
    <!-- Bootstrap CSS -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <!-- Animate.css -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css" />
    <!-- FontAwesome -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css" />
    <!-- Custom CSS -->
    <link rel="stylesheet" href="/network-management/assets/css/style.css">

    <!-- Sửa lỗi loader -->
    <style>
    #global-loader {
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
        height: 100%;
        background: rgba(255, 255, 255, 0.8);
        display: flex;
        justify-content: center;
        align-items: center;
        z-index: 9999;
        transition: opacity 0.3s;
    }

    #global-loader.hidden {
        opacity: 0;
        visibility: hidden;
    }

    .whirly-loader {
        border: 4px solid #f3f3f3;
        border-top: 4px solid #3498db;
        border-radius: 50%;
        width: 40px;
        height: 40px;
        animation: spin 1s linear infinite;
    }

    @keyframes spin {
        0% {
            transform: rotate(0deg);
        }

        100% {
            transform: rotate(360deg);
        }
    }
    </style>
</head>

<body>
    <div id="global-loader">
        <div class="whirly-loader"></div>
    </div>

    <div class="main-wrapper">
        <?php
        try {
            include __DIR__ . '/../../includes/header.php';
            include __DIR__ . '/../../includes/sidebarAll.php';
        } catch (Exception $e) {
            error_log("Lỗi khi include file: " . $e->getMessage());
            echo '<div class="alert alert-danger">Lỗi: Không thể tải giao diện</div>';
        }
        ?>

        <div class="page-wrapper">
            <div class="content">
                <div class="page-header">
                    <div class="page-title">
                        <h4>Cập nhật thông tin phân quyền</h4>
                        <h6>Trang cập nhật thông tin phân quyền thiết bị cho Admin</h6>
                    </div>
                </div>

                <!-- Toast thông báo -->
                <div class="toast-container position-fixed top-0 end-0 p-3">
                    <div id="successToast" class="toast align-items-center text-white bg-success border-0" role="alert">
                        <div class="d-flex">
                            <div class="toast-body"><?php echo htmlspecialchars($success_message); ?></div>
                            <button type="button" class="btn-close btn-close-white me-2 m-auto"
                                data-bs-dismiss="toast"></button>
                        </div>
                    </div>
                    <div id="errorToast" class="toast align-items-center text-white bg-danger border-0" role="alert">
                        <div class="d-flex">
                            <div class="toast-body"><?php echo htmlspecialchars($error_message); ?></div>
                            <button type="button" class="btn-close btn-close-white me-2 m-auto"
                                data-bs-dismiss="toast"></button>
                        </div>
                    </div>
                </div>

                <div class="card">
                    <div class="card-body">
                        <form id="updateAssignmentForm" method="POST">
                            <div class="row">
                                <div class="col-lg-3 col-sm-6 col-12">
                                    <div class="form-group">
                                        <label>Assignment ID <span class="text-danger">*</span></label>
                                        <input type="text" name="assignment_id" class="form-control" readonly required
                                            value="<?php echo htmlspecialchars($assignment_id); ?>">
                                    </div>
                                </div>
                                <div class="col-lg-3 col-sm-6 col-12">
                                    <div class="form-group">
                                        <label>Ngày trả dự kiến</label>
                                        <input type="date" name="expected_return_date" class="form-control">
                                    </div>
                                </div>
                                <div class="col-lg-3 col-sm-6 col-12">
                                    <div class="form-group">
                                        <label>Ngày trả thực tế</label>
                                        <input type="date" name="actual_return_date" class="form-control">
                                    </div>
                                </div>
                                <div class="col-lg-3 col-sm-6 col-12">
                                    <div class="form-group">
                                        <label>Trạng thái</label>
                                        <select class="form-select" name="status">
                                            <option value="update">Đã cập nhật</option>
                                            <option value="active">Đang sử dụng</option>
                                            <option value="returned">Đã trả</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-lg-12">
                                    <div class="form-group">
                                        <label>Ghi chú</label>
                                        <textarea name="notes" class="form-control"></textarea>
                                    </div>
                                </div>
                                <div class="col-lg-12">
                                    <button type="submit" class="btn btn-primary me-2">Cập nhật</button>
                                    <a href="/network-management/views/admin/assignmentlist.php"
                                        class="btn btn-secondary">Hủy</a>
                                </div>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Scripts -->
    <script src="https://code.jquery.com/jquery-3.7.1.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/feather-icons/4.29.1/feather.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jQuery-slimScroll/1.3.8/jquery.slimscroll.min.js"></script>
    <script src="https://cdn.datatables.net/1.13.4/js/jquery.dataTables.min.js"></script>
    <script src="https://cdn.datatables.net/1.13.4/js/dataTables.bootstrap5.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/apexcharts@3.44.0/dist/apexcharts.min.js"></script>
    <script src="/network-management/assets/js/script.js"></script>

    <script>
    document.addEventListener('DOMContentLoaded', () => {
        const loader = document.getElementById('global-loader');
        window.addEventListener('load', () => loader.classList.add('hidden'));

        const successToast = new bootstrap.Toast(document.getElementById('successToast'), {
            delay: 4000
        });
        const errorToast = new bootstrap.Toast(document.getElementById('errorToast'), {
            delay: 4000
        });

        <?php if ($show_success_toast): ?>
        successToast.show();
        <?php endif; ?>
        <?php if ($show_error_toast): ?>
        errorToast.show();
        <?php endif; ?>

        $("#updateAssignmentForm").on('submit', function(event) {
            event.preventDefault();
            const formData = new FormData(this);
            const submitButton = this.querySelector('button[type="submit"]');
            submitButton.disabled = true;
            submitButton.innerHTML =
                '<span class="spinner-border spinner-border-sm"></span> Đang xử lý...';

            $.ajax({
                url: window.location.pathname +
                    '<?php echo "?assignment_id=$assignment_id"; ?>',
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
                dataType: "json",
                success: function(response) {
                    submitButton.disabled = false;
                    submitButton.innerHTML = 'Cập nhật';
                    const toast = response.status === 'success' ? successToast : errorToast;
                    toast._element.querySelector('.toast-body').textContent = response
                        .message;
                    toast.show();
                    if (response.status === 'success') {
                        setTimeout(() => window.location.href =
                            '/network-management/views/admin/assignmentlist.php', 2000);
                    }
                },
                error: function(xhr) {
                    submitButton.disabled = false;
                    submitButton.innerHTML = 'Cập nhật';
                    console.error('AJAX Error:', xhr.responseText);
                    errorToast._element.querySelector('.toast-body').textContent =
                        "Lỗi hệ thống: " + (xhr.responseJSON?.message ||
                            'Không thể xử lý yêu cầu');
                    errorToast.show();
                }
            });
        });
    });
    </script>
</body>

</html>