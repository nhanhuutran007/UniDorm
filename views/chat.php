...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Kiểm tra session
    if (!isset($_SESSION['user_id']) || (isset($postData['sender_id']) && $postData['sender_id'] != $_SESSION['user_id'])) {
    ...
    error_log("POST action: $action, user_id: $userId, data: " . json_encode($postData));
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action'])) {
    ...
    // Kiểm tra session
    if (!isset($_SESSION['user_id']) || (isset($_GET['user_id']) && $_GET['user_id'] != $_SESSION['user_id'])) {
        http_response_code(403);
        echo json_encode(['success' => false, 'message' => 'Invalid session']);
        exit;
    }
    ...
    $(document).ready(function() {
        ...
        function displayMessages(messages) {
            ...
            messages.forEach(function(msg) {
                ...
                var messageHtml = `
                    <li class="${isSender ? 'chat-message-right' : 'chat-message-left'} mb-3">
                        ${!isSender ? `
                        ...
                        if (data.success && data.data) {
                            ...
                            $.each(data.data, function(userId, lastMessageTime) {
                                if (!lastCheckedTimes[userId] || lastMessageTime > (
                                    ...
                                    // Nếu người dùng đang chọn có tin nhắn mới, làm mới giao diện chat
                                    if (userId == activeUserId) {
                                        $('.chat-users-list .media.active').trigger(
                                        'click');
                                    }
                                }
                            });
